...
```math
    g(t) = t
```
* `ID = 4`: polynomial grid function of degree `p = length(c)-1` defined by its [`CamiMath.polynom`](https://walra356.github.io/CamiMath.jl/dev/#polynomials) vector ``c = [c_0, c_1,c_2,⋯\ c_p]``,
```math
